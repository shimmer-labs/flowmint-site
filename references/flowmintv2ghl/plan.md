...
**Business settings / brand controls (WALK — parked 2026-05-30):**
- **Separate business-settings page: re-scan + edit brand (colors, voice, images).** Deliberately NOT putting a color editor on the results page (too many options confuses non-technical owners — keep that page simple). Instead a separate settings surface where the owner can re-scan their site or hand-edit brand colors/voice and pick/replace images — the human-in-the-loop correction path for a bad scrape. Crib the good patterns from `~/flowmint`'s settings page (`app/routes/app.settings.tsx`): brand color picker + hex input, brand-voice display, business-model selector, status indicators. Effort: M. Impact: medium (safety net for bad color/image extraction). Phase: WALK.
